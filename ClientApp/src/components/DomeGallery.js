...
function buildItems(pool, seg) {
  ...
  while (extendedPool.length < totalSlots) {
  ...
  return coords.map((c, i) => ({
    ...
    artist: usedImages[i].artist,
    albumName: usedImages[i].albumName
  }));
}
...
export default function DomeGallery({
  ...
  useEffect(() => {
      async function fetchLikedSongs() {
          ...
          if (allImages.length < 5) {
             try {
                  ...
                  if (response.ok) {
                      ...
                      const recImages = tracks.map(track => {
                          const album = track.album || track.Album;
                          if (!album) return null;
                          const images = album.images || album.Images || [];
                          const firstImage = images[0];
                          const url = firstImage?.url || firstImage?.Url;
                          const name = track.name || track.Name;
                          const artists = (track.artists || track.Artists || []).map(a => a.name || a.Name).join(', ');
                          return {
                              src: url,
                              alt: name,
                              title: name,
                      ...
                      const uniqueRecs = recImages.filter(i => !existingSrcs.has(i.src));
                      allImages = [...allImages, ...uniqueRecs];
                  }
             } catch (e) {
          ...
          if (allImages.length < 5) {
              try {
                  ...
                  if (response.ok) {
                      ...
                      const releaseImages = albums.map(album => {
                          const images = album.images || album.Images || [];
                          const firstImage = images[0];
                          const url = firstImage?.url || firstImage?.Url;
                          const name = album.name || album.Name;
                          const artists = (album.artists || album.Artists || []).map(a => a.name || a.Name).join(', ');
                          return {
                              src: url,
                              alt: name,
                              title: name,
                      ...
                      const existingSrcs = new Set(allImages.map(i => i.src));
                      const uniqueReleases = releaseImages.filter(i => !existingSrcs.has(i.src));
                      allImages = [...allImages, ...uniqueReleases];
                  }
              } catch (e) {
          ...
          console.log("Final gallery images:", allImages);
